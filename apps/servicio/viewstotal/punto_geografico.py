...
from ..formstotal.punto_geografico import PuntoGeograficoForm as Formulario


class Home(LoginRequiredMixin, TemplateView):
    template_name = "servicio/punto_geografico/home.html"
    ...
    def get(self, request, servicio):
        ...
        if search is not None:
            objeto = Object.objects.filter(
                Q(latitud__icontains=search, servicio_id=servicio) |
                Q(longitud__icontains=search, servicio_id=servicio)
            ).order_by(order0 + 'id')
            total = objeto.count()
    ...
    def get(self, request, *args, **kwargs):
        if int(kwargs.get('pk')) == 0:
            ...
            objecdic = {
                ...
                "datos": {
                    "descripcion": str(objeto.latitud) + " ||  " + str(objeto.longitud)
                }
            }
...
class CrearObjet(LoginRequiredMixin, CreateView):
    ...
    template_name = "servicio/punto_geografico/formulario.html"
...
class DeleteObject(LoginRequiredMixin, DeleteView):
    login_url = 'login'
    model = Object
    template_name = "servicio/punto_geografico/delete_form.html"
    success_url = None
